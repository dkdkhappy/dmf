...
def load_data_set():    
    ...
    if sys.platform == 'win32':  
        dict_1.to_csv(r"tmp/korea_material_dict_syllables.txt", sep=" ", index=False, header=None)
    else:
        dict_1.to_csv("/tmp/korea_material_dict_syllables.txt", sep=" ", index=False, header=None)
    ...
    return chinese_data, purpose_list, table_6_, table_4_, rule_table3_limit_, table_5_, cites_wordlist_file, g_cites_data, chines_cites_wordlist_file, chinese_cites, hwahae_meiri2_cd, K_names, cir_safety
...
def Screening_Kor_step(df_file, K_names):
    """
    /**
    * 국문명 스크리닝 작업
    * @param {DataFrame} df 원본 데이터
...
class spell_check():
    ...
    def __init__(self, dict_path=None):
        ...
        if dict_path == None:
            diction_path = '/tmp/korea_material_dict_syllables.txt'
        else:
...
def Last_merge_file(thirdfile, ewgdata_g, hwahae_meiri2_cd, status, cir_safety):
...
def Make_output_Info(fin_cell, status):
    """
    /**
    ...
    */
    ...
    print("------------------------------------------------------------")
    for i in range(len(fin_cell.columns)):
        print(f"{i} : {fin_cell.iloc[:, i].name}")

        # status['제품타입'][0] == 'leave_on'
    print("------------------------------------------------------------")
    ...
    if (status['제품타입'][0] == 'leave_on'):
        hyg_col3 = fin_cell.iloc[:, 18].name    # 최고역사량 초과 (wash off)
        hyg_col4 = fin_cell.iloc[:, 18].name    # 최고 역사량 초과 (leave on)
        hyg_col5 = fin_cell.iloc[:, 19].name    # 챌린지 테스트 필요 여부
        hyg_col6 = fin_cell.iloc[:, 20].name    # 화장품안전기술규범대상
        hyg_col7 = fin_cell.iloc[:, 21].name    # 복합원료=1개의 원료사용목적
        hyg_col8 = fin_cell.iloc[:, 22].name    # 원료사용목적 오류
        hyg_col9 = fin_cell.iloc[:, 23].name    # 효능관련 원료사용목적 오류
        hyg_col10 = fin_cell.iloc[:, 24].name   # 화장품안전기술규범 위반 원료 사용 목적 오류
    ...
    for column_name, row in fin_cell[[hyg_col0, hyg_col1, hyg_col2, hyg_col3, hyg_col4, hyg_col5, hyg_col6, hyg_col7, hyg_col8, hyg_col9, hyg_col10]].iterrows():
        ...
        if (status['제품타입'][0] != 'leave_on' and len(row[3]) > 0):
            hygieneErrCnt2 += 1
    ...
    if (status['제품타입'][0] == 'leave_on'):
        cc_col2 = fin_cell.iloc[:, 26].name # Global CITES 경고 의심단어
        cc_col3 = fin_cell.iloc[:, 28].name # 중국위기동식물경고 의심단어

    probCcData = pd.DataFrame()
    print(cc_col0 + " / " + cc_col1 + " / " + cc_col2 + " / " + cc_col3)
    ...
    if (status['제품타입'][0] == 'leave_on'):
        mkt_col2 = fin_cell.iloc[:, 29].name # EWG
        mkt_col3 = fin_cell.iloc[:, 30].name # 화해
        mkt_col4 = fin_cell.iloc[:, 31].name # 메이리슈싱
...
def Full_code_fin(path):
    chinese_data, purpose_list, table_6_, table_4_, rule_table3_limit_, table_5_, cites_wordlist_file, g_cites_data, chines_cites_wordlist_file, chinese_cites, hwahae_meiri2_cd, K_names, cir_safety = load_data_set()
    df, status, col_length = Data_Load_process(path)
    dfile2 = Pre_data_cleaning(df)
    df, outs = Screening_Kor_step(dfile2, K_names)
    ...
    df_china_third_2 = Last_merge_file(
        df_china_third, ewgdata_g, hwahae_meiri2_cd, status, cir_safety)
    ...
    return Make_output_Info(fin_cell, status)


def Box_file_output(path):
    chinese_data, purpose_list, table_6_, table_4_, rule_table3_limit_, table_5_, cites_wordlist_file, g_cites_data, chines_cites_wordlist_file, chinese_cites, hwahae_meiri2_cd, K_names, cir_safety = load_data_set()
    df, status, col_length = Data_Load_process(path)
    dfile2 = Pre_data_cleaning(df)
    df, outs = Screening_Kor_step(dfile2, K_names)
    ...
    df_china_sort = df_china.sort_values(['순번', 'actual_wt'],  ascending=[
                                         True, False]).reset_index(drop=True)
    df_grfile = df_china_sort.groupby(['KoreanName'])['actual_wt'].sum()
    df_grfile = df_grfile.reset_index(drop = False).sort_values('actual_wt', ascending = False)    
    wrds = ', '.join(df_grfile['KoreanName'].to_list())
    df_grfile = df_grfile.reset_index(drop = True)
    df_grfile.loc[0,'단상자인풋'] = wrds
    file = os.path.basename(path)
    print(file)
    filename = os.path.join(DJANGO_DRF_FILEPOND_FILE_STORE_PATH, file)
    df_grfile.to_excel(filename)
    delete_old_files(DJANGO_DRF_FILEPOND_FILE_STORE_PATH, 1)
    return filename
