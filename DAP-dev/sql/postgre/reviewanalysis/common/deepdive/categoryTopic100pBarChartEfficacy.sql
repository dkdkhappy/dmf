/* 5. 카테고리별 / 토픽 100% 바그래프 - 바 그래프 (효능) SQL */
WITH WT_WHERE AS
    (
    ...
    (
        SELECT ROW_NUMBER() OVER(ORDER BY (SELECT SORT_KEY FROM WT_TPIC_TYPE X WHERE X.TPIC_TYPE = A.TPIC_TYPE) DESC, TPIC_ITEM COLLATE "ko_KR.utf8" DESC) AS SORT_KEY
              ,TPIC_ITEM
          FROM WT_TPIC_DATA A
    ...
    (
        SELECT X.TPIC_ITEM
              ,(SELECT SORT_KEY FROM WT_TPIC Y WHERE Y.TPIC_ITEM = X.TPIC_ITEM)       AS SORT_KEY
