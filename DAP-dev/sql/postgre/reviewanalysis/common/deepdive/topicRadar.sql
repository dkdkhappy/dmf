...
WITH WT_WHERE AS
    ...
    (
     ...
     UNION ALL
        SELECT 2   AS SORT_KEY
              ,CASE 
                 WHEN PSNG_TYPE = 'PSTV' THEN '업계평균 긍정비율' ELSE '업계평균 부정비율'
               END AS L_LGND
              ,CASE 
                 WHEN (SELECT TPIC_TYPE FROM WT_WHERE) = '전체' 
                 THEN CASE 
     ...
     UNION ALL
        SELECT 5   AS SORT_KEY
              ,CASE 
                 WHEN PSNG_TYPE = 'PSTV' THEN '카테고리 평균 긍정비율' ELSE '카테고리 평균 부정비율'
               END AS L_LGND
              ,CASE 
                 WHEN (SELECT TPIC_TYPE FROM WT_WHERE) = '전체' 
                 THEN CASE 
    ...
    (
        SELECT A.SORT_KEY
              ,A.TPIC_ITEM
              ,MAX(CASE WHEN B.SORT_KEY = 1 THEN CAST(CASE WHEN (SELECT PSNG_TYPE FROM WT_WHERE) = 'PSTV' THEN B.PSTV_RATE ELSE B.NGTV_RATE END AS DECIMAL(20,2)) END) AS PROD_RATE
              ,MAX(CASE WHEN B.SORT_KEY = 2 THEN CAST(CASE WHEN (SELECT PSNG_TYPE FROM WT_WHERE) = 'PSTV' THEN B.PSTV_RATE ELSE B.NGTV_RATE END AS DECIMAL(20,2)) END) AS ALL_RATE
              ,MAX(CASE WHEN B.SORT_KEY = 5 THEN CAST(CASE WHEN (SELECT PSNG_TYPE FROM WT_WHERE) = 'PSTV' THEN B.PSTV_RATE ELSE B.NGTV_RATE END AS DECIMAL(20,2)) END) AS CATE_RATE
    ...
    SELECT SORT_KEY
          ,TPIC_ITEM
          ,PROD_RATE  /* 제품 긍/부정 비율         */
          ,CASE WHEN (SELECT AVG_TYPE FROM WT_WHERE) = '전체' THEN ALL_RATE ELSE CATE_RATE END AS AVG_RATE  /* 전체평균/카테고리평균 비율*/
      FROM WT_RADAR
  ORDER BY SORT_KEY