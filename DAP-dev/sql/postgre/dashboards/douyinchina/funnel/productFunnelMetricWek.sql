...
WITH WT_WHERE AS
    ...
    (
        SELECT DOUYIN_ID    AS PRODUCT_ID
              ,EXPERT_NICKNAME AS PRODUCT_NM
              ,'00'                                                                                                                AS COL_WEEK
              ,SUM(live_transaction_amount)                                            AS SALE_AMT_RMB  /* 매출금액 - 위안화   */
              ,SUM(live_transaction_amount * DASH_RAW.SF_EXCH_KRW(TO_CHAR(A.DATE , 'YYYY-MM-DD')))  AS SALE_AMT_KRW  /* 매출금액 - 원화     */
              ,SUM(live_commodity_clicks)                                                                                             AS VIST_CNT      /* 방문자수            */
              ,CASE WHEN SUM(live_commodity_clicks) = 0 THEN 0 ELSE SUM(live_exposure_number)         / SUM(live_commodity_clicks)       END    AS PGVW_CNT      /* 방문자당 페이지뷰수 */
         ...
         WHERE TO_CHAR(date, 'YYYY') = (SELECT BASE_YEAR FROM WT_WHERE)
