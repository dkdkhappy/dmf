...
WITH WT_WHERE AS
    ...
    select distinct on (LIVE_NAME_CN) LIVE_NAME_CN, LIVE_NAME_KR, author_id
           from DASH_RAW.OVER_DOUYIN_LIVE_NAME
           order by LIVE_NAME_CN, LIVE_NAME_KR
    ...
    (
        SELECT DISTINCT
          ...
          INNER JOIN WT_SHOP B ON (A.NICKNAME = B.SHOP_NAME_CN)
           LEFT JOIN WT_LIVE_NAME C ON (A.NICKNAME = C.LIVE_NAME_CN)
