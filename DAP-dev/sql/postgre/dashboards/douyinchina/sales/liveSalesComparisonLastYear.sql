/* 11. 라이브별 매출 데이터 뷰어 - 전년동기간 대비 누적 매출 TOP 5 SQL      */
...
WITH WT_WHERE AS
    ...
    (
        SELECT ACCOUNT_NAME
              ...
              ,CAST(LIVE_TRANSACTION_AMOUNT_YUAN AS DECIMAL(20,4))        AS SALE_AMT_RMB   /* 매출 - 위안화 */
    ...
    (
        SELECT ACCOUNT_NAME
              ...
              ,CAST(LIVE_TRANSACTION_AMOUNT_YUAN AS DECIMAL(20,4))        AS SALE_AMT_RMB   /* 매출 - 위안화 */
          FROM DASH_RAW.OVER_{TAG}_ACCOUNT_COMPOSITION A
         WHERE TO_CHAR(CAST(CAST(DATE AS VARCHAR) AS DATE), 'YYYY-MM-DD') BETWEEN (SELECT FR_DT_YOY FROM WT_WHERE) AND (SELECT TO_DT_YOY FROM WT_WHERE)
    ), WT_EXCH AS
    (
        SELECT ACCOUNT_NAME
    ...
    (
        SELECT A.SALE_RANK                                                      /* 순위                   */
              ,COALESCE(D.ACCOUNT_NAME, '')       AS ACCOUNT_ID_YOY_RMB /* 전년 제품ID   - 위안화 */
              ,D.ACCOUNT_NAME                     AS ACCOUNT_NM_YOY_RMB /* 전년 제품명   - 위안화 */
              ,D.SALE_AMT                         AS SALE_AMT_YOY_RMB   /* 전년 매출액   - 위안화 */
              ,D.SALE_AMT / Y.SALE_AMT_RMB  * 100 AS SALE_RATE_YOY_RMB  /* 전년 매출비중 - 위안화 */
    
              ,COALESCE(B.ACCOUNT_NAME, '')       AS ACCOUNT_ID_RMB     /* 금년 제품ID   - 위안화 */
              ,B.ACCOUNT_NAME                     AS ACCOUNT_NM_RMB     /* 금년 제품명   - 위안화 */
              ,B.SALE_AMT                         AS SALE_AMT_RMB       /* 금년 매출액   - 위안화 */
              ,B.SALE_AMT / T.SALE_AMT_RMB  * 100 AS SALE_RATE_RMB      /* 금년 매출비중 - 위안화 */
    
              ,COALESCE(E.ACCOUNT_NAME, '')       AS ACCOUNT_ID_YOY_KRW /* 전년 제품ID   - 원화    */
              ,E.ACCOUNT_NAME                     AS ACCOUNT_NM_YOY_KRW /* 전년 제품명   - 원화    */
              ,E.SALE_AMT                         AS SALE_AMT_YOY_KRW   /* 전년 매출액   - 원화    */
              ,E.SALE_AMT / Y.SALE_AMT_KRW  * 100 AS SALE_RATE_YOY_KRW  /* 전년 매출비중 - 원화    */
    
              ,COALESCE(C.ACCOUNT_NAME, '')       AS ACCOUNT_ID_KRW     /* 금년 제품ID   - 원화    */
              ,C.ACCOUNT_NAME                     AS ACCOUNT_NM_KRW     /* 금년 제품명   - 원화    */
    ...
    SELECT SALE_RANK                                                                      AS SALE_RANK                                                                                            /* 순위                   */
          ,ACCOUNT_ID_YOY_RMB                                                                                   /* 전년 제품ID   - 위안화 */
          ,ACCOUNT_NM_YOY_RMB                                                             AS LIVE_NM_YOY_RMB    /* 전년 제품명   - 위안화 */
          ...
          ,ACCOUNT_ID_RMB                                                                                       /* 금년 제품ID   - 위안화 */
          ,ACCOUNT_NM_RMB                                                                 AS LIVE_NM_RMB        /* 금년 제품명   - 위안화 */
          ...
          ,ACCOUNT_ID_YOY_KRW                                                                                   /* 전년 제품ID   - 원화 */
          ,ACCOUNT_NM_YOY_KRW                                                             AS LIVE_NM_YOY_KRW    /* 전년 제품명   - 원화 */
          ...
          ,ACCOUNT_ID_KRW                                                                                       /* 금년 제품ID   - 원화 */
          ,ACCOUNT_NM_KRW                                                                 AS LIVE_NM_KRW        /* 금년 제품명   - 원화 */
