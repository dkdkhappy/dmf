...
WITH WT_WHERE AS
    ...
    (
        SELECT TO_CHAR(DATE, 'MM')  AS RANK_MNTH
              ,DOUYIN_ID
              ,EXPERT_NICKNAME
              ,CASE WHEN SUM(LIVE_COMMODITY_CLICKS) = 0 THEN 0 ELSE SUM(LIVE_TRANSACTION_NUMBER) / SUM(LIVE_COMMODITY_CLICKS) END AS ORDR_VAL  /* 구매 전환율 */
          FROM DASH_RAW.OVER_{TAG}_SHOPPING_RECOMMEND_LIVE_DETAILS A
         WHERE TO_CHAR(DATE, 'YYYY') LIKE CONCAT((SELECT BASE_YEAR FROM WT_WHERE), '%')
    ...
    (
        SELECT RANK_MNTH
              ,DOUYIN_ID
              ,EXPERT_NICKNAME
              ,RANK() OVER(PARTITION BY RANK_MNTH ORDER BY ORDR_VAL DESC, EXPERT_NICKNAME) AS ORDR_RANK  /* 구매 전환율 순위 */
              ,ORDR_VAL  /* 구매 전환율 */
          FROM WT_ORDR A
    ), WT_BASE_RANK_01 AS
    (
        SELECT 'RANK_01'                         AS RANK_TYPE  /* 순위             */
              ...
              ,EXPERT_NICKNAME                   AS PROD_NM    /* 제품명           */
          FROM WT_RANK A
         WHERE RANK_MNTH  = '01'
           AND ORDR_RANK <= 5
    ), WT_BASE_RANK_02 AS
