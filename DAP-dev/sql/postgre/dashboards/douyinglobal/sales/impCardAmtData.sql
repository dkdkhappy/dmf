...
WITH WT_WHERE AS
    ...
    (
        SELECT A.SALE_AMT_RMB                                                                               AS SALE_AMT_RMB           /* 일매출금액            - 위안화 */
              ...
              ,(CASE COALESCE(D.SALE_AMT_DOD_RMB, 0) when  0 then 0
	              else (A.SALE_AMT_RMB - COALESCE(D.SALE_AMT_DOD_RMB, 0)) / D.SALE_AMT_DOD_RMB * 100              
	              END
              )																	                            AS SALE_RATE_DOD_RMB      /* 일매출금액 증감률     - 위안화 */
              ,(CASE COALESCE(D.REFD_AMT_DOD_RMB, 0) when  0 then 0
	              else (A.REFD_AMT_RMB - COALESCE(D.REFD_AMT_DOD_RMB, 0)) / D.REFD_AMT_DOD_RMB * 100              
	              END
              )																								AS REFD_RATE_DOD_RMB      /* 일환불금액 증감률     - 위안화 */              
              ,(CASE COALESCE(D.SALE_AMT_DOD_KRW, 0) when  0 then 0
	              else (A.SALE_AMT_KRW - COALESCE(D.SALE_AMT_DOD_KRW, 0)) / D.SALE_AMT_DOD_KRW * 100                
	              END
              )																								AS SALE_RATE_DOD_KRW      /* 일매출금액 증감률     - 원화   */
              ,(CASE  COALESCE(D.REFD_AMT_DOD_KRW, 0) when  0 then 0
	              else (A.REFD_AMT_KRW - COALESCE(D.REFD_AMT_DOD_KRW, 0)) / D.REFD_AMT_DOD_KRW * 100              
	              END
              )																								AS REFD_RATE_DOD_KRW      /* 일환불금액 증감률     - 원화   */
