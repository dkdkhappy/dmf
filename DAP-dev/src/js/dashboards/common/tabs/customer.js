...
cust.dayOfWeekVisitorCountHeatMap = {}; /* 3. 요일/시간 방문자수량 히트맵 - 히트맵 SQL */
cust.dayOfWeekVisitorCountBottom = {}; /* 4. 방문자 첫방문/재방문 그래프 - 하단표 SQL */
cust.dayOfWeekVisitorCountTimeSeries = {}; /* 4. 방문자 첫방문/재방문 그래프 - 방문자 시계열 그래프 SQL */
cust.weekdayNewBuyerRatioBottom = {}; // # 5. 요일별 첫방문 구매자 비중 - 하단표 SQL weekdayNewBuyerRatioViewer
cust.weekdayNewBuyerRatioTornado = {}; /*5. 요일별 첫방문 구매자 비중 - 토네이도 그래프 SQL custAmtViewer*/
cust.visitDurationGraph = {}; /* 6. 방문자 평균체류시간 그래프 - 시계열 그래프 SQL */
cust.averageDwellTimeByDayOfWeek = {}; /* 7. 요일별 방문자 평균체류시간 - 시계열 그래프 SQL */
cust.averageRevenuePerCustomerGraph = {}; /* 8. 구매자 객단가 그래프 - 그래프 SQL */
...
cust.regionalDistributionBarChart = {}; /* 10. 지역분포 그래프 - 바 그래프 SQL */
cust.regionalDistributionMapChart = {}; /* 10. 지역분포 그래프 - 지도 그래프 SQL */
cust.genderDistributionBarChart = {}; /* 11. 성별분포 그래프 - 바 그래프 SQL */
cust.ageDistributionBarChart = {}; /* 12. 연령분포 - 바 그래프 SQL */
...
cust.setDataBinding = function () {
  ...
  /* 3 방문자 첫방문/재방문 그래프 - 방문자 시계열 그래프 SQL */
  if (Object.keys(cust.dayOfWeekVisitorCountTimeSeries).length > 0) {
    cust.dayOfWeekVisitorCountTimeSeriesUpdate();
  }
  /* 4. 방문자 첫방문/재방문 그래프 - 하단표 SQL */
  if (Object.keys(cust.dayOfWeekVisitorCountBottom).length > 0) {
    cust.dayOfWeekVisitorCountBottomUpdate();
  }
  /* 4. 요일/시간 방문자수량 히트맵 - 히트맵 SQL */
  if (Object.keys(cust.dayOfWeekVisitorCountHeatMap).length > 0) {
    cust.dayOfWeekVisitorCountHeatMapUpdate(); //
  }
  /* 5. 요일별 첫방문 구매자 비중 - 하단표 SQL */
  if (Object.keys(cust.weekdayNewBuyerRatioBottom).length > 0) {
    cust.weekdayNewBuyerRatioBottomUpdate();
  }
  /* 5. 요일별 첫방문 구매자 비중 - 토네이도 그래프 SQL */
  if (Object.keys(cust.weekdayNewBuyerRatioTornado).length > 0) {
    cust.weekdayNewBuyerRatioTornadoUpdate();
  }
  /* 6. 방문자 평균체류시간 그래프 - 시계열 그래프 SQL */
  if (Object.keys(cust.visitDurationGraph).length > 0) {
    cust.visitDurationGraphUpdate();
  }
  /* 7. 요일별 방문자 평균체류시간 - 시계열 그래프 SQL */
  if (Object.keys(cust.averageDwellTimeByDayOfWeek).length > 0) {
    cust.averageDwellTimeByDayOfWeekUpdate();
  }
...
cust.redialFirstOptions = {
  ...
  colors: getChartColorsArray("redial_first"),
};
if (document.querySelector("#redial_first")) {
  cust.redialFirstChart = new ApexCharts(document.querySelector("#redial_first"), cust.redialFirstOptions);
  cust.redialFirstChart.render();
}
...
if (document.querySelector("#redial_customer")) {
  cust.radialCustomerChart = new ApexCharts(document.querySelector("#redial_customer"), cust.redialFirstOptions);
...
cust.visitorAnalyticsCardUpdate = function () {
  // 상단 중요정보 카드
  //vist_cnt  : 전일 방문자 수          vist_cnt_mnth : 당월 방문자 수         vist_cnt_year  : 당해 방문자 수
  //vist_rate : 전일 방문자 수 증감률   vist_rate_mnth : 당월 방문자 수 증감률  vist_rate_year  : 당해 방문자 수 증감률
  //cust_amt  : 객단가                 stay_time      :평균 체류시간          frst_rate_yoy : 첫 방문자 비율             paid_rate : 구매자 비율
  //cust_rate : 객단가 증감률           stay_rate      : 평균 체류시간 증감률
  const cardAreaList = [
    "vist_cnt",
    "vist_rate",
    "vist_cnt_mnth",
  ...
  cardAreaList.forEach((cardArea) => {
    ...
    if (el) {
      if (cardArea.indexOf("vist_rate") > -1 || cardArea.indexOf("cust_rate") > -1 || cardArea.indexOf("stay_rate") > -1) {
        ...
        const recentData = cust.visitorAnalyticsCard[0][`${cardArea}`];
        if (elArrow) {
          ...
          if (cardArea.indexOf("cust") > -1) {
            el.innerText = cust.visitorAnalyticsCard[0][`${cardArea}_${currency}`] + "%";
          } else {
            el.innerText = cust.visitorAnalyticsCard[0][`${cardArea}`] + "%";
          }
        }
      } else if (cardArea.indexOf("cust") > -1) {
        const dataTarget = cust.visitorAnalyticsCard[0][`${cardArea}_${currency}`];
        el.innerText = 0;
        el.setAttribute("data-target", dataTarget);
      } else {
        const dataTarget = cust.visitorAnalyticsCard[0][`${cardArea}`];
        el.innerText = 0;
        el.setAttribute("data-target", dataTarget);
      }
    }
  ...
  if (cust.redialFirstChart) {
    cust.redialFirstChart.updateOptions({
      series: [cust.visitorAnalyticsCard[0]["frst_rate"]],
    });
  }

  if (cust.redialRebuyChart) {
    cust.redialRebuyChart.updateOptions({
      series: [cust.visitorAnalyticsCard[0]["repd_rate"]],
    });
  }

  if (cust.radialCustomerChart) {
    cust.radialCustomerChart.updateOptions({
      series: [cust.visitorAnalyticsCard[0]["paid_rate"]],
    });
  }
};
 ...
 * 3. 요일/시간 방문자수량 히트맵
 */
cust.dayOfWeekVisitorCountHeatMapUpdate = function () {
  if (cust.heatMapTimeSale) {
    let rawData = cust.dayOfWeekVisitorCountHeatMap;
    cust.heatMapTimeSale.setOption(cust.heatMapTimeSaleOption, true);
    if (rawData.length > 0) {
      // 전일 매출 그래프 데이터 업데이트
      let dayOfWeekVisitorCountHeatMap = [];
      dayOfWeekVisitorCountHeatMap = cust.dayOfWeekVisitorCountHeatMap.map(({ week_no, hour_no, vist_cnt }) => [Number(week_no), Number(hour_no), Number(vist_cnt)]);

      let maxVal = cust.dayOfWeekVisitorCountHeatMap.reduce(function (prev, current) {
        return Number(prev.vist_cnt) > Number(current.vist_cnt) ? prev : current;
      }).vist_cnt;

      cust.data = dayOfWeekVisitorCountHeatMap.map(function (item) {
        return [item[1], item[0], item[2] || "-"];
      });
      cust.heatMapTimeSale.setOption({
        visualMap: {
          min: 0,
          max: maxVal,
        },
...
if (document.getElementById("heat-map-time-custs")) {
  cust.heatMapTimeSale = echarts.init(document.getElementById("heat-map-time-custs"));
  cust.heatMapTimeSale.setOption(cust.heatMapTimeSaleOption);
}

/******************************************************************************************************************************/

/************4. 방문자 첫방문/재방문 그래프 - 하단표 SQL ********************************************************************/
cust.dayOfWeekVisitorCountBottomUpdate = function () {
  if (cust.firstVisitReVisitListGrid) {
    let filterData = [];
    for (var i = 0; i < cust.dayOfWeekVisitorCountBottom.length; i++) {
      filterData.push(Object.values(cust.dayOfWeekVisitorCountBottom[i]));
    }
    cust.firstVisitReVisitListGrid.updateConfig({ data: filterData }).forceRender();
  }
};
// 방문자 첫 방문 / 재 방품 데이터 뷰어
if (document.getElementById("firstVisitReVisitList")) {
  cust.firstVisitReVisitListGrid = new gridjs.Grid({
    columns: [
      {
        name: "구분",
        width: "100px",
  ...
  }).render(document.getElementById("firstVisitReVisitList"));
}

// 4. 방문자 첫방문/재방문 그래프 - 방문자 시계열 그래프 SQL
cust.dayOfWeekVisitorCountTimeSeriesUpdate = function () {
  if (cust.charMixVisitRevisit) {
    let rawData = cust.dayOfWeekVisitorCountTimeSeries;
    let visitData = [];
    let visitWeekData = [];
    let visitMnthData = [];
    let frstData = [];
    ...
    for (const data of cust.dayOfWeekVisitorCountTimeSeries) {
      if (data.l_lgnd_id === "VISIT") {
        visitData.push(data);
      } else if (data.l_lgnd_id === "VISIT_WEEK") {
        visitWeekData.push(data);
      } else if (data.l_lgnd_id === "VISIT_MNTH") {
        visitMnthData.push(data);
      } else if (data.l_lgnd_id === "FRST") {
        frstData.push(data);
      } else if (data.l_lgnd_id === "PAID") {
        paidData.push(data);
      }
    }
    ...
    cust.charMixVisitRevisitOption = {
      legend: {
        data: legend,
        selected: {
          "주 방문자수": false,
    ...
    cust.charMixVisitRevisit.setOption(cust.charMixVisitRevisitOption);
  }
};
...
if (document.getElementById("chart-mix-visit-revisit")) {
  cust.charMixVisitRevisit = echarts.init(document.getElementById("chart-mix-visit-revisit"));
  cust.charMixVisitRevisit.setOption(cust.charMixVisitRevisitOption);
}

/****************************************************************************************************************************************************************/
/* 5. 요일별 첫방문 구매자 비중 - 하단표 SQL */
cust.weekdayNewBuyerRatioBottomUpdate = function () {
  if (cust.dayFirstVisitBuyListGrid) {
    let filterData = [];
    for (var i = 0; i < cust.weekdayNewBuyerRatioBottom.length; i++) {
      filterData.push(Object.values(cust.weekdayNewBuyerRatioBottom[i]));
    }
    cust.dayFirstVisitBuyListGrid.updateConfig({ data: filterData }).forceRender();
  }
};
// 요일별 첫 방문
if (document.getElementById("dayFirstVisitBuyList")) {
  cust.dayFirstVisitBuyListGrid = new gridjs.Grid({
    columns: [
      {
        name: "구분",
        width: "100px",
  ...
  }).render(document.getElementById("dayFirstVisitBuyList"));
}

/* 5. 요일별 첫방문 구매자 비중 - 토네이도 그래프 SQL */
cust.weekdayNewBuyerRatioTornadoUpdate = function () {
  if (cust.charTornDay1stVisitBuy) {
    let rawData = cust.weekdayNewBuyerRatioTornado;
    cust.charTornDay1stVisitBuy.setOption(cust.charTornDay1stVisitBuyOption, true);
    if (rawData.length > 0) {
      let frstData = [];
      let paidData = [];
      //let xWeeks = [];
      ...
      for (const data of rawData) {
        if (data.l_lgnd_id === "FRST") {
          frstData.push(data);
        } else if (data.l_lgnd_id === "PAID") {
          paidData.push(data);
        }
      }
      let dataSum = frstData.length + paidData.length;
      cust.charTornDay1stVisitBuy.setOption({
        tooltip: {
          trigger: "axis",
          axisPointer: {
            type: "shadow",
          ...
          formatter: function (params) {
            ...
            var value1 = typeof params[0].value != "undefined" ? params[0].value.toString() : "0";
            var name1 = params[0].seriesName;
            var color2 = params[1].color;
            var value2 = Math.abs(params[1].value);
            var name2 = params[1].seriesName;
            var param1 =
              '<span style="display:inline-block;width:10px;border-radius:50%;height:10px;background-color:' +
              color1 +
              ';margin-right:5px;"></span>' +
              name1 +
              '<span style="font-weight:900;float:right;margin-left:20px;font-size:14px;color:#666;">' +
              value1 +
              "</span>";
            var param2 =
              '<span style="display:inline-block;width:10px;border-radius:50%;height:10px;background-color:' +
        ...
        series: [
          {
            ...
            data: paidData.map((item) => item.x_val),
          },
          {
            name: "첫 방문",
            type: "bar",
            ...
            data: frstData.map((item) => Number(item.x_val) * -1),
          },
        ],
        graphic: {
          elements: [
...
cust.charTornDay1stVisitBuyOption = {
  tooltip: {
    trigger: "axis",
    axisPointer: {
      type: "shadow",
...
if (document.getElementById("chart-Torn-day-1st-visit-buy")) {
  cust.charTornDay1stVisitBuy = echarts.init(document.getElementById("chart-Torn-day-1st-visit-buy"));
  cust.charTornDay1stVisitBuy.setOption(cust.charTornDay1stVisitBuyOption);
}

/***********************************************************************************************************/
/* 6. 방문자 평균체류시간 그래프 - 시계열 그래프 SQL */
cust.visitDurationGraphUpdate = function () {
  if (cust.charLineVisitStayTime) {
    let rawData = cust.visitDurationGraph;
    cust.charLineVisitStayTime.setOption(cust.charLineVisitStayTimeOption, true);
    if (rawData.length > 0) {
      let data1 = [];
      let data2 = [];
      ...
      for (const data of rawData) {
        if (data.l_lgnd_id === "VIST_YOY") {
          data1.push(data.y_val);
        } else if (data.l_lgnd_id === "VIST") {
          data2.push(data.y_val);
        }
      }
      ...
      let dataSum = data1.length + data2.length;

      // 2023-04-04 양윤지 legend 추가
      let legendData = [];
      ...
      cust.charLineVisitStayTime.setOption({
        legend: {
          data: legendData,
          textStyle: {
            color: "#858d98",
...
cust.charLineVisitStayTimeOption = {
  tooltip: {
    trigger: "axis",
  },
  grid: {
...
if (document.getElementById("chart-line-visit-stay-time")) {
  cust.charLineVisitStayTime = echarts.init(document.getElementById("chart-line-visit-stay-time"));
  cust.charLineVisitStayTime.setOption(cust.charLineVisitStayTimeOption);
}

/***********************************************************************************************************/
/* 7. 요일별 방문자 평균체류시간 - 시계열 그래프 SQL */
cust.averageDwellTimeByDayOfWeekUpdate = function () {
  if (cust.charLineVisitDayStayTime) {
    let rawData = cust.averageDwellTimeByDayOfWeek;
    cust.charLineVisitDayStayTime.setOption(cust.charLineVisitDayStayTimeOption, true);
    if (rawData.length > 0) {
      let datas = [];
      for (let i = 0; i < cust.days.length; i++) {
        let sum = 0,
          count = 0;
        for (const data of cust.averageDwellTimeByDayOfWeek) {
          if (data.sort_key === i) {
            sum += data.y_val;
            count++;
          }
      ...
      cust.charLineVisitDayStayTime.setOption({
        xAxis: {
          type: "category",
          data: cust.days,
        },
...
cust.charLineVisitDayStayTimeOption = {
  tooltip: {
    trigger: "axis",
  },
  toolbox: {
...
if (document.getElementById("chart-line-visit-day-stay-time")) {
  cust.charLineVisitDayStayTime = echarts.init(document.getElementById("chart-line-visit-day-stay-time"));
  cust.charLineVisitDayStayTime.setOption(cust.charLineVisitDayStayTimeOption);
}
...
cust.buyerTimeSeriesCardUpdate = function () {
  if (document.getElementById("time_series_paid_cnt")) {
    document.getElementById("time_series_paid_cnt").setAttribute("data-target", cust.buyerTimeSeriesCard[0]["paid_cnt"]);
    document.getElementById("time_series_paid_cnt").innerText = 0;
  }

  if (document.getElementById("time_series_paid_cnt_yoy")) {
    document.getElementById("time_series_paid_cnt_yoy").setAttribute("data-target", cust.buyerTimeSeriesCard[0]["paid_cnt_yoy"]);
    document.getElementById("time_series_paid_cnt_yoy").innerText = 0;
  }

  if (document.getElementById("time_series_paid_rate")) {
    document.getElementById("time_series_paid_rate").innerText = cust.buyerTimeSeriesCard[0]["paid_rate"] + "%";
  }
};
...
cust.resizeHandlers = [
  cust.chartLineVisit,
  cust.heatMapTimeSale,
  cust.charMixVisitRevisit,
  cust.charTornDay1stVisitBuy,
  cust.charLineVisitDayStayTime,
  cust.chartLineBuyer,
  cust.charMixBuyRebuy,
  cust.charMixCustPrice,
  cust.charLineVisitStayTime,
...
cust.onLoadEvent = function (initData) {
    ...
    onChange: function (selectedDates, dateStr, instance) {
      if (selectedDates.length > 1) {
        ...
        let params = {
          ...
          tab: "customer",
          //dataList: ["visitorTimeSeriesChart", "dayOfWeekVisitorCountHeatMap", "dayOfWeekVisitorCountTimeSeries"],
          dataList: ["visitorTimeSeriesChart", "dayOfWeekVisitorCountHeatMap"],
        };
        getData(params, function (data) {
          cust.visitorTimeSeriesChart = {};
          cust.dayOfWeekVisitorCountHeatMap = {};
          cust.dayOfWeekVisitorCountTimeSeries = {};
          ...
          /* 4. 요일/시간 방문자수량 히트맵 - 히트맵 SQL */
          if (data["dayOfWeekVisitorCountHeatMap"] != undefined) {
            cust.dayOfWeekVisitorCountHeatMap = data["dayOfWeekVisitorCountHeatMap"];
            cust.dayOfWeekVisitorCountHeatMapUpdate();
          }

          /* 3 방문자 첫방문/재방문 그래프 - 방문자 시계열 그래프 SQL */
          /*
          if (data["dayOfWeekVisitorCountTimeSeries"] != undefined) {
            cust.dayOfWeekVisitorCountTimeSeries = data["dayOfWeekVisitorCountTimeSeries"];
            cust.dayOfWeekVisitorCountTimeSeriesUpdate();
          }
          */
        });
  ...
  // 방문자 첫 방문 / 재 방문 그래프 - flatpickr 이벤트 dayOfWeekVisitorCountViewer
  let dayOfWeekVisitorCountViewer = flatpickr("#dayOfWeekVisitorCountViewer, #dayFirstVisitorBuyerViewer", {
    locale: "ko", // locale for this instance only
    defaultDate: `${initData.fr_dt} ~ ${initData.to_dt}`,
    mode: "range",
    onChange: function (selectedDates, dateStr, instance) {
      if (selectedDates.length > 1) {
        ...
        dayOfWeekVisitorCountViewer[0].setDate([fromDate, toDate]);
        dayOfWeekVisitorCountViewer[1].setDate([fromDate, toDate]);
        ...
        let params = {
          ...
          menu: "dashboards/common",
          tab: "customer",
          dataList: ["dayOfWeekVisitorCountTimeSeries", "weekdayNewBuyerRatioTornado"],
        };
        getData(params, function (data) {
          cust.dayOfWeekVisitorCountTimeSeries = {};
          cust.weekdayNewBuyerRatioTornado = {};
          /* 방문자 첫 방문 / 재 방문 그래프 */
          if (data["dayOfWeekVisitorCountTimeSeries"] != undefined) {
            cust.dayOfWeekVisitorCountTimeSeries = data["dayOfWeekVisitorCountTimeSeries"];
            cust.dayOfWeekVisitorCountTimeSeriesUpdate();
          ...
          /* 요일별 첫 방문 구매자 비중 */
          if (data["weekdayNewBuyerRatioTornado"] != undefined) {
            cust.weekdayNewBuyerRatioTornado = data["weekdayNewBuyerRatioTornado"];
            cust.weekdayNewBuyerRatioTornadoUpdate();
          }
        });
      }
  ...
  //방문자 평균 체류시간 그래프- flatpickr 이벤트
  let weekdayNewBuyerRatioViewer = flatpickr("#weekdayNewBuyerRatioViewer, #dayVisitorAutoStayTimeViewer", {
    locale: "ko", // locale for this instance only
    defaultDate: `${initData.fr_dt} ~ ${initData.to_dt}`,
    mode: "range",
    onChange: function (selectedDates, dateStr, instance) {
      if (selectedDates.length > 1) {
        ...
        weekdayNewBuyerRatioViewer[0].setDate([fromDate, toDate]);
        weekdayNewBuyerRatioViewer[1].setDate([fromDate, toDate]);
        ...
        let params = {
          ...
          menu: "dashboards/common",
          tab: "customer",
          dataList: ["visitDurationGraph", "averageDwellTimeByDayOfWeek"],
        };
        getData(params, function (data) {
          cust.visitDurationGraph = {};
          cust.averageDwellTimeByDayOfWeek = {};
          /* 방문자 평균 체류시간 그래프 */
          if (data["visitDurationGraph"] != undefined) {
            cust.visitDurationGraph = data["visitDurationGraph"];
            cust.visitDurationGraphUpdate();
          }
          /* 요일별 방문자 평균 체류시간 그래프 */
          if (data["averageDwellTimeByDayOfWeek"] != undefined) {
            cust.averageDwellTimeByDayOfWeek = data["averageDwellTimeByDayOfWeek"];
            cust.averageDwellTimeByDayOfWeekUpdate();
          }
        });
      }
  ...
  // 구매자 첫 구매 / 재 구매 그래프 - flatpickr 이벤트
  flatpickr("#dayOfWeekBuyerCountViewer", {
    locale: "ko", // locale for this instance only
    defaultDate: `${initData.fr_dt} ~ ${initData.to_dt}`,
    mode: "range",
    onChange: function (selectedDates, dateStr, instance) {
  ...
  let dataList = [
    ...
    "visitorTimeSeriesBottom" /* 2. 방문자수 시계열 그래프 - 하단표 SQL */,
    "dayOfWeekVisitorCountHeatMap" /* 3. 요일/시간 방문자수량 히트맵 - 히트맵 SQL */,
    "dayOfWeekVisitorCountTimeSeries" /* 4. 방문자 첫방문/재방문 그래프 - 방문자 시계열 그래프 SQL */,
    "dayOfWeekVisitorCountBottom" /* 4. 방문자 첫방문/재방문 그래프 - 하단표 SQL */,
    "weekdayNewBuyerRatioBottom", // # 5. 요일별 첫방문 구매자 비중 - 하단표 SQL
    "weekdayNewBuyerRatioTornado" /*5. 요일별 첫방문 구매자 비중 - 토네이도 그래프 SQL */,
    "visitDurationGraph" /* 6. 방문자 평균체류시간 그래프 - 시계열 그래프 SQL */,
    "averageDwellTimeByDayOfWeek" /* 7. 요일별 방문자 평균체류시간 - 시계열 그래프 SQL */,
    "buyerTimeSeriesCard" /* 8. 구매자 수 시계열 그래프 - 그래프상단 정보 SQL */,
    "buyerTimeSeriesChart" /* 8. 구매자 수 시계열 그래프 - 시계열그래프 SQL */,
    "buyerTimeSeriesBottom" /* 8. 구매자 수 시계열 그래프 - 하단표 SQL */,
    "buyerFirstBuyRebuyChart" /* 9. 구매자 첫구매/재구매 비율 - 시계열 그래프 SQL */,
    "buyerFirstBuyRebuyBottom" /* 9. 구매자 첫구매/재구매 비율 - 하단 표 SQL */,
    "buyerProfitGraph" /* 9. 구매자당 수익 그래프 - 바 그래프 SQL */,
    "averageRevenuePerCustomerGraph" /* 10. 구매자 객단가 그래프 - 그래프 SQL */,
    "regionalDistributionBarChart" /* 11. 지역분포 그래프 - 바 그래프 SQL */,
    "regionalDistributionMapChart" /* 11. 지역분포 그래프 - 지도 그래프 SQL */,
    "genderDistributionBarChart" /* 12. 성별분포 그래프 - 바 그래프 SQL */,
    "ageDistributionBarChart" /* 13. 연령분포 - 바 그래프 SQL */,
  ];
