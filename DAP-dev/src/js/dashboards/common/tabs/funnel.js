...
funnel.setDataBinding = function () {
  ...
  if (Object.keys(funnel.uniqueChannelVisitors).length > 0) {
    funnel.uniqueChannelVisitorsUpdate();
  }
  /* 2. Unique Visitor (UV) 추이 분석 - 표 SQL */
  if (Object.keys(funnel.yearlyUniqueChannelVisitors).length > 0) {
    funnel.yearlyUniqueChannelVisitorsUpdate();
  }
...
/********************************************** Unique Visitor (UV) **********************************************/
funnel.uniqueChannelVisitorsUpdate = function () {
  let rawData = funnel.uniqueChannelVisitors;
...
funnel.yearlyUniqueChannelVisitorsUpdate = function () {
  let rawData = funnel.yearlyUniqueChannelVisitors;
  let keysToExtract = [
    "row_titl",
    "vist_cnt_01",
    "vist_cnt_02",
  ...
  if (funnel.channelVisitorList) {
    funnel.channelVisitorList.updateConfig({ data: filterData }).forceRender();
  }
};
/* 당해 연도 채널 방문자 데이터 뷰어 */
if (document.getElementById("channelVisitorList")) {
  funnel.channelVisitorList = new gridjs.Grid({
    columns: [
      {
        name: "구분",
        width: "100px",
    ...
    data: [],
  }).render(document.getElementById("channelVisitorList"));
}
/*************************************************************************************************************/
          ...
          series: [
            {
              ...
              data: [
                ...
                { value: rawData.filter((item) => item.lgnd_id == "VIST")[0]["step_cnt"], name: "방문" },
              ],
            },
          ],
        },
...
funnel.productPCCRCumulativeYoYUpdate = function () {
  ...
  let keysToExtract = ["ordr_rank", "prod_nm_yoy", "ordr_val_yoy", "ordr_rate_yoy", "prod_nm", "ordr_val", "ordr_rate"];
  let filterData = [];
  for (var i = 0; i < rawData.length; i++) {
    filterData.push(keysToExtract.map((key) => rawData[i][key]));
  }
...
funnel.onLoadEvent = function (initData) {
  ...
  flatpickr(
    "#funnelProductVisitDatepicker, #funnelProdPageViewDatePicker, #funnelProdFunnelDatepicker, #funnelProdFunnelMetricsDatepicker, #funnelProdVisitPageViewDatepicker",
    {
  ...
  flatpickr(
    "#funnelChannelVisitDatepicker, #funnelChannelPageViewDatepicker, #funnelChannelVisitPageViewDatepicker",
    {
      locale: "ko", // locale for this instance only
      defaultDate: `${initData.fr_dt} ~ ${initData.to_dt}`,
      mode: "range",
      onChange: function (selectedDates, dateStr, instance) {
        if (selectedDates.length > 1) {
          ...
          switch (elId) {
            case "funnelChannelVisitDatepicker":
              // Unique Visitor (UV)
              params["dataList"] = ["uniqueChannelVisitors"];
              break;
            case "funnelChannelPageViewDatepicker":
              // Page View (PV)
              params["dataList"] = ["channelPageViews"];
              break;
          ...
          getData(params, function (data) {
            switch (elId) {
              case "funnelChannelVisitDatepicker":
                funnel.uniqueChannelVisitors = {};
                if (data["uniqueChannelVisitors"] != undefined) {
                  funnel.uniqueChannelVisitors = data["uniqueChannelVisitors"];
                  funnel.uniqueChannelVisitorsUpdate();
                }
                break;
              case "funnelChannelPageViewDatepicker":
  ...
  let funnelDatePicker2 = flatpickr("#funnelChannelFunnelDatepicker, #funnelChannelChangeDatepicker", {
      locale: "ko", // locale for this instance only
      defaultDate: `${initData.fr_dt} ~ ${initData.to_dt}`,
      mode: "range",
      onChange: function (selectedDates, dateStr, instance) {
        if (selectedDates.length > 1) {
          ...
          let params = {
            ...
            menu: "dashboards/common",
            tab: "funnel",
            dataList: ["channelFunnelAnalysis", "channelConversionRateAnalysis"]
          };
  ...
  flatpickr("#lastMonthStoreFunnelFlatpickr", {
    disableMobile: "true",
    locale: "ko", // locale for this instance only
    plugins: [
    ...
    onChange: function (selectedDates, dateStr, instance) {
      ...
      let params = {
        ...
        menu: "dashboards/common",
        tab: "funnel",
        dataList: ["storeFunnelMetricMoM"],
      };
      getData(params, function (data) {
        funnel.storeFunnelMetricMoM = {};
            if (data["storeFunnelMetricMoM"] != undefined) {
              funnel.storeFunnelMetricMoM = data["storeFunnelMetricMoM"];
  ...
  if (document.getElementById("storeFunnelListInfo")) {
    ...
    storeFunnelListInfo.addEventListener("change", function (val) {
      ...
      getData(params, function (data) {
        ...
        let lastYearMonth1 = document.getElementById("last_year_month_store");
        switch (type) {
          case "yoy":
  ...
  let dataList = [
    ...
    "uniqueChannelVisitors" /* 1. Unique Visitor (UV) - 시계열 그래프 SQL */,
    "yearlyUniqueChannelVisitors" /* 2. Unique Visitor (UV) 추이 분석 - 표 SQL */,
